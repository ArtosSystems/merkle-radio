...
object BeatMaker {
  ...
  case class Note(frequency: Double, duration: Double) {
    ...
    def `2M`(direction: Direction) = gap(3)
    def `3m`(direction: Direction) = gap(4)
    def `3M`(direction: Direction) = gap(5)
    def `4`(direction: Direction) = gap(6)
    def `5dim`(direction: Direction) = gap(7)
    def `5j`(direction: Direction) = gap(8)
    def `6m`(direction: Direction) = gap(9)
    def `6M`(direction: Direction) = gap(10)
    def `7m`(direction: Direction) = gap(11)
    def `7M`(direction: Direction) = gap(12)
